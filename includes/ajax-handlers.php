...
/**
 * Handle quick comment AJAX request
 */
add_action( 'wp_ajax_bpplg_quick_comment', 'bp_plg_ajax_quick_comment' );
function bp_plg_ajax_quick_comment() {
	...
	if ( ! $user_id ) {
		wp_send_json_error( array( 'message' => __( 'You must be logged in to comment.', 'bp-page-like-groups' ) ) );
	}

	// Verify activity exists
	$activity = bp_activity_get_specific( array( 'activity_ids' => $activity_id ) );
	if ( empty( $activity['activities'] ) ) {
		wp_send_json_error( array( 'message' => __( 'Activity not found.', 'bp-page-like-groups' ) ) );
	}

	// Check if user can comment on this activity
	if ( ! bp_activity_can_comment() ) {
		wp_send_json_error( array( 'message' => __( 'Comments are disabled for this activity.', 'bp-page-like-groups' ) ) );
	}
	...
	if ( $comment_id ) {
		...
		bp_plg_increment_activity_engagement( $activity_id );

		// Get the new comment data to return
		$comment = bp_activity_get_specific( array( 
			'activity_ids' => $comment_id,
			'display_comments' => false 
		) );

		wp_send_json_success( array(
			'comment_id' => $comment_id,
			'message' => __( 'Quick reaction added!', 'bp-page-like-groups' )
		) );
	} else {
		wp_send_json_error( array( 'message' => __( 'Failed to add reaction. Please try again.', 'bp-page-like-groups' ) ) );
	}
}
 ...
 */
add_action( 'wp_ajax_bpplg_track_view', 'bp_plg_ajax_track_view' );
add_action( 'wp_ajax_nopriv_bpplg_track_view', 'bp_plg_ajax_track_view' );
function bp_plg_ajax_track_view() {
	check_ajax_referer( 'bpplg-nonce', 'nonce' );
	...
	// Increment views
	$new_views = bp_plg_increment_activity_views( $activity_id );
